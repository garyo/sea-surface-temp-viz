...
- **`export_og_cards.py` renders globe-viz's link previews.** One 1200×630 JPEG per time series under `maps/og/`, named `{source}-{dataset}-{region}.jpg`, plus `manifest.json` holding each card's date and preview text. globe-viz's Pages Function builds that key straight from a shared URL's `src`/`ds`/`region` params, so the ids here are a contract with the site. A new dataset needs an entry in `DATASET_LABELS` (it's skipped otherwise), and a new region should get a `GLOBE_VIEWS` framing. `upload-to-s3.py` sets `ContentType` from the extension because crawlers reject an `og:image` served as S3's default `binary/octet-stream`.
